"""Тестирование всех 23 tools в режиме заглушек."""
import asyncio
import os
import sys
...
async def test_all_23_tools():
    """Тестирование всех 23 tools в режиме заглушек."""
    print("=" * 80)
    print("🧪 ТЕСТИРОВАНИЕ ВСЕХ 23 TOOLS В РЕЖИМЕ ЗАГЛУШЕК (TEST)")
    print("=" * 80)
    ...
    tests = [
        ...
        ("submit_declaration_to_fns", submit_declaration_to_fns, {
            "declaration_xml": "<xml>test</xml>"
        }),
        # API-ФНС методы
    ...
    for name, tool_func, args in tests:
        try:
            print(f"\n{'='*80}")
            print(f"🔍 Тест {passed + failed + 1}/23: {name}")
            args["ctx"] = ctx
            ...
            if result and result.content:
                ...
                # Проверяем, что это заглушка
                if "тестовый режим" in text_preview.lower() or "заглушка" in text_preview.lower() or "тестов" in text_preview.lower():
                    print(f"   ✓ Используется заглушка (test режим)")
                else:
    ...
    print(f"   📈 Всего протестировано: {passed + failed}/23")
    print("=" * 80)
    
    return failed == 0

if __name__ == "__main__":
    success = asyncio.run(test_all_23_tools())
